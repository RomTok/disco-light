...
*/
class MMSImage : public MMSWidget {
    private:
        string         className;
        MMSImageClass  *imageClass;
        MMSImageClass  myImageClass;
        ...
        void loadMyImage(string path, string filename, MMSFBSurface **surface, MMSIM_DESC_SUF **surfdesc, unsigned int *index);

        void workWithRatio(MMSFBSurface *suf, DFBRectangle *surfaceGeom);
        
    public:
        MMSImage(MMSWindow *root, string className, MMSTheme *theme = NULL);
        ~MMSImage();
        MMSWIDGETTYPE getType() { return MMSWIDGETTYPE_IMAGE; }
    ...
    public:
        ...
        void updateFromThemeClass(MMSImageClass *themeClass);

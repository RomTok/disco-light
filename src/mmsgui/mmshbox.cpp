...
MMSHBox::MMSHBox(MMSWindow *root) : MMSWidget::MMSWidget() {
    create(root);
}

bool MMSHBox::create(MMSWindow *root) {
    return MMSWidget::create(root, false, true, false, false, true, true);
}

MMSWidget *MMSHBox::copyWidget() {
    /* create widget */
    MMSHBox *newWidget = new MMSHBox(this->rootwindow); 

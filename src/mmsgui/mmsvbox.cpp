...
MMSVBox::MMSVBox(MMSWindow *root) : MMSWidget::MMSWidget() {
    create(root);
}
 
bool MMSVBox::create(MMSWindow *root) {
    return MMSWidget::create(root, false, true, false, false, true, true);
}

MMSWidget *MMSVBox::copyWidget() {
    /* create widget */
    MMSVBox *newWidget = new MMSVBox(this->rootwindow); 
